...
def main():
    st.set_page_config(
        page_title= "Simple Resume Analyzer",
        layout= "wide"
    )
    ...
    This Simple Resume Analyzer was developed to analyze the resumes and to select the desired candidate(s) 
    from a bunch of resmues by passing the folder path bellow and puch on the bellow buttons to get result.""")
    image = mp.imread("resume-parser.png")
    st.image(image)
    ...
    with st.form(key="form1"):
        ...
        name=st.text_input(label= "Enter the folder path")
    ...
    # when 'Predict' is clicked, make the prediction and store it 
        ...
        # Search the directory path and loop through the resume documents and call the function getText
    ...
    for filename in os.listdir(path):
        ...
        peoplesoft = (ResumeText["RoleApplied"] == "Peoplesoft")
    ...
    if load or st.session_state.load_state:
        ...
        if opt == "Pie chart":
            ...
            with col5:
                ...
                fig.update_layout(title= "Sum of Role Applied regarding Skills")
                st.plotly_chart(fig)
            with col6:
    ...
    lod = st.button("Check Visualizations of Experience ")
    choice = lod
    if choice == lod:
        ...
        if lod or st.session_state.lod_state:
            ...
            st.subheader(body="Take a look at Experience(years) & Skills")
    ...
    if daol or st.session_state.daol_state:
        ...
        with col11:
            fig= px.density_heatmap(df,x ="experience", y="skills",z="role")
            fig.update_layout(title="Heatmap of Experience(years) & skills of selected candidate")
            st.plotly_chart(fig)
        with col12:
            fig= px.scatter(df,x="experience",y="skills")
            fig.update_layout(title="Scatterplot of Experience(years) & skills of selected candidate")
            st.plotly_chart(fig)
    ...
    with col16:
        if st.button("Peoplesoft Resumes"):
            st.dataframe(ResumeText[peoplesoft])
    with col17:
        if st.button("Workday Resumes"):
            st.dataframe(ResumeText[workday])


#Resumes = r"C:\Users\Mahesh\Downloads\Project\Resumes\\"
#new_dir = r"C:\Users\Mahesh\Downloads\Project\\"
